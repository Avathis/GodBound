...
#include "GB_TATrace.h"
#include "AbilitySystemComponent.h"
#include "DrawDebugHelpers.h"
#include "GameFramework/PlayerController.h"
#include "GameplayAbilitySpec.h"

AGB_TATrace::AGB_TATrace()
{
...
float AGB_TATrace::GetCurrentSpread() const
{
...
void AGB_TATrace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void AGB_TATrace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void AGB_TATrace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bInDestroyOnConfirmation;
}

void AGB_TATrace::StartTargeting(UGameplayAbility* Ability)
{
...
void AGB_TATrace::ConfirmTargetingAndContinue()
{
...
void AGB_TATrace::CancelTargeting()
{
...
void AGB_TATrace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void AGB_TATrace::Tick(float DeltaSeconds)
{
...
void AGB_TATrace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World, const FGameplayTargetDataFilterHandle FilterHandle, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params)
{
	check(World);
...
bool AGB_TATrace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter, float AbilityRange, FVector& ClippedPosition)
{
...
void AGB_TATrace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle AGB_TATrace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> AGB_TATrace::PerformTrace(AActor* InSourceActor)
{
...
AGameplayAbilityWorldReticle* AGB_TATrace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void AGB_TATrace::DestroyReticleActors()
{
